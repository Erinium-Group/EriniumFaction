...
public class PlayerProtectionProcedure {
	public static boolean execute(LevelAccessor world, double x, double z, Entity entity, String perm) {
		...
		if (entity instanceof ServerPlayer) {
			if (entity.getData(EriniumFactionModVariables.PLAYER_VARIABLES).faction_bypass) {
				return true;
			} else if (!(tempId).equals(entity.getData(EriniumFactionModVariables.PLAYER_VARIABLES).faction) && !(tempId).equals("wilderness")) {
				return false;
			}
		}
		if ((tempId).equals("safezone") || (tempId).equals("warzone")) {
			if ((perm).equals("canBreak") || (perm).equals("canPlace")) {
				return false;
			}
			return true;
			...
			if (file.exists()) {
				...
				if (needcheckworldfile) {
					file = CurrentChunkFileProcedure.execute(world, x, z);
					{
						try {
							BufferedReader bufferedReader = new BufferedReader(new FileReader(file));
							StringBuilder jsonstringbuilder = new StringBuilder();
							...
							if (jsonObject.has(perm)) {
								if (jsonObject.get(perm).getAsString().contains(factionRank)) {
									impossibleToInterract = false;
								} else {
									impossibleToInterract = true;
								}
