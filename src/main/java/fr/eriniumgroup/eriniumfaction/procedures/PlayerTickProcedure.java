...
public class PlayerTickProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double z, Entity entity) {
		...
		com.google.gson.JsonObject secJson = new com.google.gson.JsonObject();
		if (!world.isClientSide()) {
			...
			if (new Object() {
				private int getPlayerTickCount(Entity entity1) {
					if (entity1 instanceof Player player) {
						return player.tickCount;
					}
					return 0;
				}
			}.getPlayerTickCount(entity) % 6000 == 0) {
				if (entity.getData(EriniumFactionModVariables.PLAYER_VARIABLES).power + 1 <= (double) FactionConfigServerConfiguration.POWERPERPLAYER.get()) {
					{
						EriniumFactionModVariables.PlayerVariables _vars = entity.getData(EriniumFactionModVariables.PLAYER_VARIABLES);
						_vars.power = entity.getData(EriniumFactionModVariables.PLAYER_VARIABLES).power + 1;
						_vars.syncPlayerVariables(entity);
					}
					if (HaveAFactionProcedure.execute(entity)) {
						file = FactionFileByIdProcedure.execute(entity.getData(EriniumFactionModVariables.PLAYER_VARIABLES).faction);
						{
							try {
								...
								bufferedReader.close();
								secJson = new com.google.gson.Gson().fromJson(jsonstringbuilder.toString(), com.google.gson.JsonObject.class);
								secJson.addProperty("power", (secJson.get("power").getAsDouble() + 1));
								{
									com.google.gson.Gson mainGSONBuilderVariable = new com.google.gson.GsonBuilder().setPrettyPrinting().create();
									try {
										FileWriter fileWriter = new FileWriter(file);
										fileWriter.write(mainGSONBuilderVariable.toJson(secJson));
										fileWriter.close();
